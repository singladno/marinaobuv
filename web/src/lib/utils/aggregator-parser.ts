...
import { logger as pino } from '@/lib/server/logger';

/** Pino typings reject `debug(string, ...args)`; bundle extras as the object payload. */
const logger = {
...
function convertToOrigUrl(url: string): string {
  try {
    ...
    logger.error('Error converting URL to orig:', error, 'Original URL:', url);
...
export function parseAggregatorHTML(html: string): ParsedAggregatorData {
  logger.debug('[AGGREGATOR PARSER] Starting HTML parsing...');
  logger.debug('[AGGREGATOR PARSER] HTML length:', html.length);
  logger.debug(
  ...
  allImages.each((index, el) => {
    ...
    logger.debug(`[AGGREGATOR PARSER] Image ${index + 1} src:`, src);
    logger.debug(
  ...
  logger.debug('[AGGREGATOR PARSER] Total images extracted:', images.length);
  if (images.length > 0) {
    logger.debug('[AGGREGATOR PARSER] Image URLs:', images);
  } else {
    logger.debug('[AGGREGATOR PARSER] WARNING: No images found!');
  ...
  labelElements.each((index, el) => {
    ...
    logger.debug(`[AGGREGATOR PARSER] Label ${index + 1}:`, labelText);
    if (labelText) {
      labels.push(labelText);
    }
  });
  logger.debug('[AGGREGATOR PARSER] Total labels extracted:', labels.length);
  ...
  if (postCaptLink.length > 0) {
    ...
    logger.debug('[AGGREGATOR PARSER] Provider href:', href);
    if (href) {
    ...
    logger.debug('[AGGREGATOR PARSER] Provider location:', providerLocation);
  } else {
    logger.debug('[AGGREGATOR PARSER] No provider link found');
  }
  ...
  logger.debug('[AGGREGATOR PARSER] Provider name:', providerName);
  ...
  logger.debug('[AGGREGATOR PARSER] Price text:', priceText);
  if (priceText) {
    // Extract numbers from price text (e.g., "1000 руб" -> 1000)
    const priceMatch = priceText.match(/(\d+)/);
    if (priceMatch) {
      price = parseInt(priceMatch[1], 10);
      logger.debug('[AGGREGATOR PARSER] Price extracted:', price);
    } else {
      logger.debug('[AGGREGATOR PARSER] No numbers found in price text');
    }
  } else {
    logger.debug('[AGGREGATOR PARSER] No price text found');
  }
  ...
  sizeElements.each((index, el) => {
    ...
    logger.debug(`[AGGREGATOR PARSER] Size ${index + 1}:`, sizeText);
    if (sizeText) {
      sizes.push(sizeText);
    }
  });
  logger.debug('[AGGREGATOR PARSER] Total sizes extracted:', sizes.length);
  ...
  logger.debug('[AGGREGATOR PARSER] Parsing complete. Summary:');
  logger.debug(
    '[AGGREGATOR PARSER] - Text info:',
    textInfo ? `${textInfo.length} chars` : 'EMPTY'
  );
  logger.debug('[AGGREGATOR PARSER] - Images:', images.length);
  logger.debug('[AGGREGATOR PARSER] - Labels:', labels.length);
  logger.debug('[AGGREGATOR PARSER] - Provider link:', providerLink || 'NONE');
  logger.debug(
    '[AGGREGATOR PARSER] - Provider location:',
    providerLocation || 'NONE'
  );
  logger.debug('[AGGREGATOR PARSER] - Provider name:', providerName || 'NONE');
  logger.debug('[AGGREGATOR PARSER] - Price:', price || 'NONE');
  logger.debug('[AGGREGATOR PARSER] - Sizes:', sizes.length, sizes);
