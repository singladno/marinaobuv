...
/**
 * Fetch messages from the specified group chat with improved pagination and time handling
 */
type FetchOptions = {
...
export async function fetchGroupMessagesImproved(
  chatId: string,
  limit: number = 50,
  ...
  const maxMessages = options?.maxMessages || 10000; // Increased limit
  ...
  while (
    ...
    if (messages.length === 0) {
      ...
      if (consecutiveEmptyBatches >= 2) {
        console.log('Received 2 consecutive empty batches, stopping');
        hasMore = false;
        break;
      }
    } else {
