...
- ПРАВИЛА ОПРЕДЕЛЕНИЯ СЕЗОНА (КРИТИЧЕСКИ):
  * Текст поставщика важнее фото: «осень», «Осень 2026», «осенние» → AUTUMN, даже если на фото кажется мех или байка
  * «зима» / «зимние» в тексте → WINTER; «лето» → SUMMER; «весна» → SPRING
  * Если есть слово "мех" на изображении → сезон "WINTER" ТОЛЬКО если в тексте нет другого сезона
  * Анализируйте стиль обуви для определения сезона (открытая vs закрытая, толщина и т.д.) только если сезон не назван в тексте
...
- Если в тексте «цвета разные», «2 цвет», «3 цвет», «N цвет» при N≥2 — в описание ОБЯЗАТЕЛЬНО фраза «В упаковке разные цвета»
...
- Сезон: сначала слово из текста поставщика, иначе стиль обуви на фото
...
Верните JSON с полной информацией о товаре:
{
  ...
  "materialDetails": "1-2 предложения о материале",
  "gender": "MALE/FEMALE",
  "season": "SPRING/SUMMER/AUTUMN/WINTER",
  "color": "основной цвет",
  "visualDetails": "1-2 предложения: форма, стиль, детали",
  "features": "застежки, подошва, утепление — коротко",
  "additionalNotes": "только если есть важное, иначе пустая строка",
  "price": null | number
}

ПРАВИЛА ДЛЯ ПОЛЯ "price":
- Поле "price" является ОПЦИОНАЛЬНЫМ
- Возвращайте "price" ТОЛЬКО если в тексте контекста упомянута ОПТОВАЯ цена (например: "цена оптом", "оптовая цена", "цена опт", "опт")
- Если оптовая цена указана в тексте - извлеките её числовое значение и верните в поле "price"
- Если оптовая цена НЕ указана в тексте - НЕ возвращайте поле "price" или верните null
- Примеры: "цена оптом: 140руб" → price: 140, "оптовая цена 200 руб" → price: 200
- Игнорируйте розничные цены (например, "цена штучно", "цена за штуку") - извлекайте ТОЛЬКО оптовую цену

- Поле "material" — ТОЛЬКО 1-2 слова. Детали — в materialDetails (1-2 предложения).
- description: 2-3 предложения, без воды и без копирования текста поставщика.
- Остальные текстовые поля — по 1-2 предложения. Не раздувайте ответ.`
...
- Поле "material" — 1-2 слова. Остальные поля — коротко, без воды.
...
- ОПТОВАЯ ЦЕНА: если в тексте упомянута оптовая цена (например: "цена оптом", "оптовая цена", "цена опт", "опт"), извлеките её числовое значение и верните в поле "price". Если оптовая цена НЕ указана - НЕ возвращайте поле "price" или верните null.
