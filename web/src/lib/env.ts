...
const schema = z.object({
  ...
  WHAPI_WEBHOOK_SECRET: z.string().min(32, 'WHAPI_WEBHOOK_SECRET must be at least 32 characters'),
  ...
  MESSAGE_FETCH_HOURS: z.string().optional().transform((val) => {
    if (!val) return 24; // Default to 24 hours
    const parsed = parseInt(val, 10);
    if (isNaN(parsed) || parsed < 1) {
      throw new Error('MESSAGE_FETCH_HOURS must be a positive integer');
    }
    return parsed;
  }),
}).refine(
