...
export function parseFilters(searchParams: URLSearchParams): CatalogFilters {
  ...
  let sort = (searchParams.get('sort') as CatalogFilters['sort']) ?? 'relevance';
  if (!['relevance', 'price-asc', 'price-desc', 'newest'].includes(sort)) sort = 'relevance';
...
export function buildQueryString(next: Partial<CatalogFilters>, prev?: CatalogFilters): string {
  ...
  if (merged.priceFrom != null) params.set('priceFrom', String(merged.priceFrom));
  ...
  if (merged.pageSize && merged.pageSize !== 24) params.set('pageSize', String(merged.pageSize));
