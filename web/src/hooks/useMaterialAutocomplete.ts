...
export function useMaterialAutocomplete(value: string) {
  ...
  const fetchSuggestions = useCallback(async (query: string) => {
    ...
    try {
      const response = await fetch(
        `/api/admin/products/materials?q=${encodeURIComponent(query)}`,
        {
      ...
      if (!isInitialLoadRef.current && data.materials && data.materials.length > 0) {
        setIsOpen(true);
      }
    } catch (err) {
  ...
  const handleSelect = useCallback(
    (material: string, onSelect: (material: string) => void) => {
