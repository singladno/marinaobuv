...
interface OrderEditableLabelProps {
  order: AdminOrder;
  onUpdate: (orderId: string, updates: Partial<AdminOrder>) => Promise<void>;
}

export function OrderEditableLabel({
  order,
  onUpdate,
}: OrderEditableLabelProps) {
  const { addNotification } = useNotifications();
  const [value, setValue] = React.useState(order.label || '');
  ...
  const handleBlur = async () => {
    ...
    if (trimmed === (order.label || null)) return;

    setIsUpdating(true);
    setStatus('saving');

    try {
      await onUpdate(order.id, { label: trimmed as any });
      setStatus('success');
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === 'Escape') setValue(order.label || '');
  };
