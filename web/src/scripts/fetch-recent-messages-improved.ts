...
/**
 * Fetch messages from the last N hours with improved logic
 */
async function fetchRecentMessagesImproved(
  chatId: string
  ...
  console.log(
    `Time range: ${new Date(hoursAgo * 1000).toISOString()} to ${new Date().toISOString()}`
  );

  // Fetch messages with a higher limit to ensure we get all messages
  // Increase limit to 5000 to handle high-volume groups
  const messages = await fetchGroupMessages(chatId, 5000);
  ...
  const recentMessages = messages.filter(message => {
    ...
    // Only include messages from the time range
    const isInTimeRange = messageTimestamp >= hoursAgo;

    if (!isInTimeRange) {
      console.log(
        `Skipping message from ${new Date(messageTimestamp * 1000).toISOString()} (outside ${fetchHours}h range)`
      );
    }
  ...
  console.log(
    `Found ${recentMessages.length} messages from the last ${fetchHours} hours (filtered from ${messages.length} total)`
  );
...
async function main() {
  ...
  try {
    ...
    console.log('Starting improved recent message fetch...');
    console.log(`Target group ID: ${env.TARGET_GROUP_ID}`);

    // Fetch recent messages with improved logic
    const messages = await fetchRecentMessagesImproved(env.TARGET_GROUP_ID);
