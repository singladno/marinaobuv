...
async function main() {
  ...
  // Demo product under Зима/Женская обувь/Зимние кроссовки
  const demo = {
  ...
  const winterWomensSneakers = await prisma.category.findUnique({
    where: { slug: 'winter-womens-sneakers' },
  });
  if (!winterWomensSneakers)
    throw new Error("Category 'winter-womens-sneakers' not found");
